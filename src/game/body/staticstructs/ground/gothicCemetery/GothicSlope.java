 ...
 */
// Class
abstract class GothicSlope extends GroundFrame { // this class is witchcraft, all you have to know is that it is reactive to the image and lengthScale :))))
    // Fields
    ...
    GothicSlope(GameWorld gameWorld, float x, float y, int lengthScale, Direction direction, GameBodyImage bodyImage, int divVal) {
        ...
        float[] vertices = getFloats(lengthScale, direction);
    ...
    private float[] getFloats(int lengthScale, Direction direction) {
        float[] vertices;
        switch (direction) {
            case DOWN -> {
